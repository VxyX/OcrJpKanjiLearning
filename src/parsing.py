...
class Parse():
    def __init__(self):
        ...
        self.unidicD = os.path.join(os.path.dirname(filepath), 'dic', 'unidic').replace('\\','/')
        self.mecabRc = os.path.join(os.path.dirname(filepath), 'mecab', 'etc', 'mecabrc').replace('\\','/')
        # self.ipadic = os.path.join(os.path.dirname(filepath), 'dic', 'ipadic').replace('\\','\\\\')
        # print(self.unidicD)
        self.tagger = MeCab.Tagger(f"-r {self.mecabRc} -d {self.unidicD}")
    ...
    def convKanaRo(text, jenis_huruf='hiragana'or'katakana'or'romaji'):
        ...
        if jenis_huruf == 'romaji':
            new_txt = pykakasi.kakasi().convert(text)
            new_txt = new_txt[0]['hepburn']
        else:
            for char in txt:
                char_code = ord(char)
            ...
            for index, c in enumerate(kanjilist):
                ...
                if (not (0x3040 <= c_code <= 0x309F) and not (0x30A0 <= c_code <= 0x30FF)):
                    ...
                    if (last_kanji):
                        ...
                        # check if word with kanji and hiragana reading have the same char
                            if (c == c1):
                                # check if 
                                if (index1 != 0):
                                    str_hi = ''
                                    ...
                                    continue
                                # this will remove the current word and current index
            ...
            try:
                ...
                try:
                    if '萌える' in data[i][3] and ((data[i+1][4] != '助動詞' or  'デス' in data[i+1][5]) and not ('接続助詞' in data[i+1][4])):
                        ...
                        combine_katadasar = False
                        katadasar = ['萌え', 'もえ']
                    if '何時' in data[i][3] and ('も' in data[i+1][3] and '助詞-係助詞' in data[i+1][4]):
                        conj = True
                        combine_katadasar = True
                except Exception as e:
                    pass
                ...
                if combine_katadasar:
                    ...
                    for c in range(len(temp_h)):
                        ...
                        if c == "ー":
                            ...
                            # print(data[i][0], kanalist, c_index)
                            if tes[-1] == 'i' or tes[-1] == 'e':
                                c = 'い'
                                try:
                                    if tes[-1] == self.convKanaRo(data[i][2][c_index+1], 'romaji'):
...
if (__name__ == "__main__"):
    ...
    # print(filepath)
    parse = Parse()
    a = parse.segmentasiTeks("ひと泳ぎしよう")
    # a = parse.convKanaRo('キョ','romaji')
    # a = ['','助動詞']
    # if ('助詞' in a[1]):
    #     print(a)
